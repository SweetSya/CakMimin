...
function createNewsigModal_edit(jml_data) {
    ...
    <div class="edit-prod-wrap">
        ...
        <div action="`+act_where_2+`" method="POST" class="form-edit-prod">
        <input type="hidden" name="_token" value="`+csrf_token+`" />     
            <div>
                <h1>Hapus Data</h1>
                <div>
                ...
                <button class="hapus-newsig">Hapus</button>
            </div>
        </div>
    </div>
    `
    ...
    document.querySelector('.hapus-newsig').onclick = () => {
        form = document.createElement('form')
        form.setAttribute('action', act_where_2)
        ...
        form.innerHTML = `
            ...
            <div>
                <h1>Hapus Data</h1>
                <div>
                    `+getNewsigDelLayout(newsig_to_delete)+`
                </div>
                <button class="hapus-ne">Hapus</button>
            </div>
        `
        edit_elem.appendChild(form)
        form.submit()
...
function getNewsigDelLayout(data) {
    var rt = ''
    for(var x = 0; x < data.length; x++) {
        rt = rt+ `
        <span> 
            <span><input class="newsig-id-to-delete" name="link[`+x+`]" type="text" value="`+newsig_to_delete[x]+`" readonly> <img src="`+pub_url_img+`/delete.png" class="newsig-id-delete" style="width:30px;height:30px;background-color: rgba(var(--color_white), 1); border-radius: 100%; padding: 5px;"></span>
        </span>
        `
    }
    return rt
